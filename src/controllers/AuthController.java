...
public class AuthController {
	...
	private String getUsername(String add) {
		String username =add;
		    System.out.println(username.replaceAll("((@.*)|[^a-zA-Z])+", " ").trim());
		return username;
	}
    @FXML
    	...
    	Credentials cr = new Credentials(un.getText(),pwd.getText());
    	  //Authentication.connect(un.getText(), pwd.getText())
    			if(cr.auth()) {
    				...
    				String username = this.getUsername(un.getText());
    				Connection conn= DBConnect.getConnection(); 
    				String sql = "insert into userinf(Name, Email, connected, password) values ('"+username+"','"+un.getText()+"', 'true','"+pwd.getText()+"');";
